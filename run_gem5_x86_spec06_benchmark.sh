...
GEM5_DIR=/nobackup/l2_repl_policy/l2_gem5							# Install location of gem5
SPEC_DIR=/nobackup/					# Install location of your SPEC2006 benchmarks
##################################################################
...
RUN_DIR=$SPEC_DIR/$BENCHMARK_CODE/run/run_base_ref\_i386-m32-gcc42-nn.0000		# Run directory for the selected SPEC benchmark
...
#in this way of launching gem5, we first fast forward through 10 billion instructions and
#then simulate one billion instructions. 
#$GEM5_DIR/build/X86/gem5.fast --outdir=$OUTPUT_DIR $GEM5_DIR/configs/example/spec06_config.py --benchmark=$BENCHMARK --benchmark_stdout=$RUN_DIR/$OUTPUT_DIR/$BENCHMARK.out --benchmark_stderr=$RUN_DIR/$OUTPUT_DIR/$BENCHMARK.err  --bp-type=LTAGE --fast-forward=10000000000 -I 1000000000 --caches --l2cache --cpu-type=DerivO3CPU --mem-size=16GB    | tee $SCRIPT_OUT
#this command-line is only for the sake of taking checkpoints after fast forward of
#some number of instructions. 
#$GEM5_DIR/build/X86/gem5.fast --outdir=$OUTPUT_DIR $GEM5_DIR/configs/example/spec06_config.py --benchmark=$BENCHMARK --benchmark_stdout=$RUN_DIR/$OUTPUT_DIR/$BENCHMARK.out --benchmark_stderr=$RUN_DIR/$OUTPUT_DIR/$BENCHMARK.err  --checkpoint-dir=checkpoint_$OUTPUT_DIR --checkpoint-at-end --bp-type=LTAGE  -I 10000000000 --caches --l2cache --cpu-type=AtomicSimpleCPU --mem-size=16GB    | tee $SCRIPT_OUT
#this command-line is to be used if we want to restore the application from the created 
#checkpoint
$GEM5_DIR/build/X86/gem5.fast --outdir=$OUTPUT_DIR $GEM5_DIR/configs/example/spec06_config.py --restore-with-cpu=AtomicSimpleCPU --benchmark=$BENCHMARK --benchmark_stdout=$RUN_DIR/$OUTPUT_DIR/$BENCHMARK.out --benchmark_stderr=$RUN_DIR/$OUTPUT_DIR/$BENCHMARK.err --checkpoint-dir=checkpoint_$CHECKPOINT_DIR --caches --l2cache --cpu-type=AtomicSimpleCPU -I 1000000000 --mem-size=16GB --checkpoint-restore=1  
#$GEM5_DIR/build/X86/gem5.debug --outdir=$OUTPUT_DIR $GEM5_DIR/configs/example/spec06_config.py --benchmark=$BENCHMARK --benchmark_stdout=$RUN_DIR/$OUTPUT_DIR/$BENCHMARK.out --benchmark_stderr=$RUN_DIR/$OUTPUT_DIR/$BENCHMARK.err  --bp-type=LTAGE --fast-forward=1000 -I 100000 --caches --l2cache  --cpu-type=DerivO3CPU --mem-size=16GB    | tee $SCRIPT_OUT
