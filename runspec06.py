...
def RunAndGetTicks(cliargs, benchmark, outdir, pipe_to_file=True):
    if pipe_to_file:
        proc = subprocess.Popen(cliargs, stdout=open(spec_home_directory +
            benchmark_directories[benchmark]+
            '/run/run_base_ref_i386-m32-gcc42-nn.0000/'+
            outdir + '/stdout.txt', 'w+'), stderr=open(spec_home_directory +
                benchmark_directories[benchmark]+
...
def RunBenchmark(benchmark_name):
    runtype = '_l1_miss_repl_policy_stats_1B'
    checkpoint_dir = '_VC_DSR_O3CPU_squashed_load_analysis'
#    runtype = '_pointer_chased_loads_ref'
    target_path = spec_home_directory + benchmark_directories[benchmark_name]+
    '/run/run_base_ref_i386-m32-gcc42-nn.0000/'+ benchmark_name+runtype
    #print(target_path)
...
#benchmark_names = ['milc','soplex','sphinx3','povray','zeusmp','GemsFDTD',
#'lbm','bwaves','namd','calculix','leslie3d','gromacs','bzip2','mcf','hmmer',
#'libquantum','h264ref','omnetpp','astar','sjeng','gobmk']
#benchmark_names = ['milc','soplex','sphinx3','povray','zeusmp','GemsFDTD',
#'lbm','bwaves','namd','calculix','leslie3d','gromacs','mcf','hmmer',
#'libquantum','h264ref','omnetpp','astar','sjeng','gobmk']
benchmark_names = [ 'sphinx3']
